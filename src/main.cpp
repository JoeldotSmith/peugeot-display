...
static uint32_t apply_clock_offset(uint32_t counterSeconds) {
  int32_t adjusted = (int32_t)(counterSeconds % 86400UL) + clockOffsetSeconds;
  adjusted %= 86400;
...
void handle_clock() {
  if (current.secondsOfDay != latest.secondsOfDay ||
    ...
    char buf[16];
    if (latest.hasClock) {
      format_clock(buf, sizeof(buf), apply_clock_offset(latest.secondsOfDay));
